...
size_t palindrome_partitioning2_DP_topdown_optimized_helper_it(
    const PalindromeSubstringCacheIterative& B,
  ...
  for (size_t i = start_idx; i < size; i++)
  {
    if (B.is_palindrome(start_idx, i))  // O(1)
      ans = std::min(
          ans,
          1
              + palindrome_partitioning2_DP_topdown_optimized_helper_it(
                  B, i + 1, memoization_cache));
  }
...
size_t palindrome_partitioning2_DP_topdown_optimized_it(const std::string s)
{
  ...
  Cache memoization_cache;
  return palindrome_partitioning2_DP_topdown_optimized_helper_it(
      B, 0u, memoization_cache);
}
