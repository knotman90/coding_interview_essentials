...
TEST(two_numbers_problem, test1)
{
    for(const std::tuple<std::vector<int>,int,bool> p : {
        ...
        std::make_tuple(std::vector<int>({9, 4, 17, 42, 36, -3, 15}),14,true),
        std::make_tuple(std::vector<int>({9, 4, 17, 42, 36, -3, 15}),17,false),
    })
    {
            ...
            const auto& expected = std::get<2>(p);
        {
            ...
            ASSERT_EQ(expected, two_numers_sum_bruteforce(input, T)) <<"failed for "<<to_string(input)<< " and "<<T;
        }
        {
            const auto input = std::get<0>(p);
            ASSERT_EQ(expected, two_numers_sum_hashset(input, T)) <<"failed for "<<to_string(input)<< " and "<<T;
        }

        {
            auto input = std::get<0>(p);
            ASSERT_EQ(expected, two_numers_sum_sorting(input, T)) <<"failed for "<<to_string(input)<< " and "<<T;
        }
        {
            auto input = std::get<0>(p);
            ASSERT_EQ(expected, two_numers_sum_two_pointers(input, T)) <<"failed for "<<to_string(input)<< " and "<<T;
        }
