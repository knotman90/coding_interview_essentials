...
TEST(valid_parenthesis, test_all_valid_no_asteriscs)
{
  ...
  const auto all_valid =
      Book::Algorithm::generate_well_parenthesized_combinations(num_pairs);
...
TEST(valid_parenthesis, test_all_valid_substitute_chars_with_asteriscs)
{
  ...
  {
    const int num_substitutions =
        Book::Algorithm::get_random_in_range<int>(0, s.size() - 1);
...
TEST(
    valid_parenthesis,
    test_all_valid_append_closed_make_string_invalid_but_with_asteriscs_before_becomes_valid)
{
  ...
  for (auto s : all_valid)
  {
    ...
    const int num_ast =
        Book::Algorithm::get_random_in_range<int>(1, s.size() - 1);
