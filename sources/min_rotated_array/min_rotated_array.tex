...
\lstinputlisting[language=c++, caption=One-linear brute force (using C++ STL).,label=list:min_rotated_array_bf]{sources/min_rotated_array/min_rotated_array_solution1.cpp}
...
The first of the two solutions uses the \inline{std::min_element} function, while the second explicitly loops over all the elements of the array, keeping track of the smallest examined. 
