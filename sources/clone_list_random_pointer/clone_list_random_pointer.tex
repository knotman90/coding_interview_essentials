...
Linked list and arrays are possibly one of the most known data structures, and they somehow are very similar and dissimilar at the same time: both organize data in a linearly and yet they do it by taking two radically different approaches where the former arrange data in seemingly random places in memory that are then linked one another, while the latter uses a chunk of contiguous memory.

In this chapter we are going to look at an interesting problem on a strange and unusual type of linked list that is basically a singly linked list, where each node has with an additional pointer that \textbf{might} point to any another node in the list and you are asked to create a copy of this structure.

The C++ definition of said list is given in Listing \ref{list:clone_list_random_pointer:list_definition}. We can see that \inline{Node} contains an additional field \inline{random} which constitute the only dissimilarity from a canonical linked list definitions (like the ones we have used in Chapter \ref{ch:delete_duplicates_list} and Listing \ref{list:delete_duplicates_list:linked_list}).
...
class Node
{
  ...
  T val;
  Node *next{nullptr};
  // pointer to a random node in the lsit
  Node *random{nullptr};
...
\begin{exercise}
Wirte a function that, given a linked list with random pointer (defined in Listing \ref{list:clone_list_random_pointer:list_definition}) returns a deep-copy of its input.
