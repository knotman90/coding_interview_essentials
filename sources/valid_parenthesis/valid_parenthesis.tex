...
\lstinputlisting[language=c++, caption={Brute-force, exponential time solution to the problem of validating a string of parenthesis with wild-cards.},label=list:valid_parenthesis:bruteforce]{/home/dspataro/git/algorithm_articles/sources/valid_parenthesis/valid_parenthesis_solution1.cpp}
...
The DP solution is definitely a better solution than the brute-force but it is still far from the optimal. In the next section we will investigate a much faster solution that has the additional benefit of being also much shorter in length and therefore less error prone than the ones presented so far.

\lstinputlisting[language=c++, caption={Dynamic programming solution to the problem of validating a string of parenthesis with wild-cards.},label=list:valid_parenthesis:dp]{/home/dspataro/git/algorithm_articles/sources/valid_parenthesis/valid_parenthesis_solution2.cpp}

Also note that the structure \inline{pair_hash} is necessary so that the \inline{std::unordered_map} knows how to  correctly calculate a hash value for  \inline{std::pair}s. 
...
\begin{itemize}
	...
	\item an asterix \fbox{*} we can choose to either consider this as an open parenthesis (increasing the max number of obtainable open ones), delete it (leaving the balance unvaried) or convert it into a closed parenthesis (reducing \inline{min} )
\end{itemize}
...
\lstinputlisting[language=c++, caption={Linear time constant space solution to the problem of validating a string of parenthesis with wild-cards.},label=list:valid_parenthesis:linear]{/home/dspataro/git/algorithm_articles/sources/valid_parenthesis/valid_parenthesis_solution3.cpp}
