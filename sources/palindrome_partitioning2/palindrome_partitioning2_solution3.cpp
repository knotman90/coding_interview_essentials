...
size_t palindrome_partitioning2_DP_topdown_optimized_helper(
    const PalindromeSubstringCacheRecursive& B,
    ...
    Cache& memoization_cache)
{
  ...
  for (size_t i = start_idx; i < size; i++)
  {
    if (B.is_palindrome(start_idx, i))  // O(1)
      ans = std::min(ans,
                     1
                         + palindrome_partitioning2_DP_topdown_optimized_helper(
...
size_t palindrome_partitioning2_DP_topdown_optimized(const std::string s)
{
  ...
  Cache memoization_cache;
  return palindrome_partitioning2_DP_topdown_optimized_helper(
