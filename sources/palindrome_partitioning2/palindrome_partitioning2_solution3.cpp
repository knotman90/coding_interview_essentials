...
int palindrome_partitioning2_DP_bottomup(const std::string s)
{
  ...
  for(int idx_start = s.size()-2 ; idx_start >=0 ; idx_start--)
  {
      if(is_palindrome(s, idx_start, s.size()-1) ){
        DP[idx_start] = 0;
        continue;
      }
      for(int idx_end = idx_start ; idx_end < s.size() ; idx_end++){
          if(is_palindrome(s, idx_start, idx_end)){
              const auto cost_partition_rest = idx_end < s.size()-1 ? DP[idx_end+1] : 0;
