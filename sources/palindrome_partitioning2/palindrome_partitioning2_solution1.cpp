...
/**
 * Returns
 * 	 true iff the substring of s starting at start and ending at end is palindrome
 *   false otherwise.
 * */
...
int palindrome_partitioning2_bruteforce(const std::string s)
{
	...
	for(int k = 1 ; k <= std::ssize(s)-1; k++)
	{
		...
		//is there a partition of size k such that all the associated substrings in s are palindrome?
		const auto found = std::any_of(std::begin(cutpoints), std::end(cutpoints), [&](const auto& combo)
		{
