...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{/home/dspataro/git/algorithm_articles/sources/can_jump/images/can_jump_example1}
...
\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{/home/dspataro/git/algorithm_articles/sources/can_jump/images/example2}
...
from cell $N$ to itself is not to jump at all). For all other cells we have that:
	...
	\lstinputlisting[language=c++, caption={Exponential time solution to the \textit{jump game} problem where only forward jumps are performed.},label=list:can_jump1_1]{/home/dspataro/git/algorithm_articles/sources/can_jump/can_jump_solution1_1.cpp}
...
\lstinputlisting[language=c++, caption={Quadratic time and linear space DFS solution to the \textit{jump game} problem using a visited array.},label=list:can_jump1]{/home/dspataro/git/algorithm_articles/sources/can_jump/can_jump_solution1.cpp}
...
\lstinputlisting[language=c++, caption={Greedy solution where we use the fact that the DP solution described by Equation \ref{eq:can_jump:dpformula} can be optimized if we only consider if it is possible to reach the closest cell from which we can jump to the end of the array. },label=list:can_jump]{/home/dspataro/git/algorithm_articles/sources/can_jump/can_jump_solution2.cpp}
