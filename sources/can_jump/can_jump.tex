...
where there we are the only player and we are initially located at the first cell of the array.
Our goal is to get to the last cell by jumping from one cell to another. The values inside the array's cells contain the
information on the length of the jump we can take from each cell. 

There are a number of possible different solutions to this problem and, in this Chapter, we will discuss the most common. 
In particular:
\begin{itemize}
...
The function should return true if you are able to reach the last cell of the array, otherwise it should returns false.
...
As the implicit graph is not guaranteed to be acyclic, in order to make this
...
Another way of looking at this problem is by thinking of the cells in being divided in levels and in order to solve this problem we need to apply a BFS visit to the cells. 
