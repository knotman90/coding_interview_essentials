...
\begin{QandA}
	\item \begin{questionitem} \begin{question} What should the function \inline{add} return if the stream counted less than $K$ elements?  \end{question} 	 
    \begin{answered}
		\textit{You can assume that the largest $k^{th}$ elements exists when \inline{add} is
		called.}
	\end{answered} \end{questionitem}

	\item \begin{questionitem} \begin{question} Is there a limit to the size of the array $I$?  \end{question} 	 
    \begin{answered}
		\textit{No.}
	\end{answered} \end{questionitem}
...
ordered multiset (we use the \CC STL implementation named \inline{std::multiset}). 
