...
The problem presented in this chapter is another classic that often appears during interviews. It can also be considered as an evolution of the problem of finding the minimum element in a sorted and rotated array which was covered in chapter \ref{ch:min_rotated_array} at page \pageref{ch:min_rotated_array}. The two problems are so inherently linked that it is actually possible to solve this problem by using the other's solution structure. 
