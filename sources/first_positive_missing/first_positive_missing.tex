...
Listing \ref{list:first_positive_missing_bruteforce} shows an implementation of this idea where we use \inline{std::find()} as a means to do the actual search in the $A$. 
...
Notice that:
\begin{itemize}
	\item the iterator \inline{it} always points to the currently evaluated element. 
	...
	\end{itemize}
	\inline{it} is moved to its initial location by using the \inline{std::find_if} function from the STL which runs in linear time. We might as well have used binary search to perform this task, but that would have not really helped us lowering the overall asymptotic time complexity, because the sorting operation itself costs $O(nlog(n))$ and the subsequent \inline{while} loop runs in linear time. 
...
In order not to use additional space, we have to somehow implement the functionality the array \inline{F} (in the code above) provides using the input array itself! Doing so sounds harder than it really is, especially until we realize that we can store the information about a positive number being pres
