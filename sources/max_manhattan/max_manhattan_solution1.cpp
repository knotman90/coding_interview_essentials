//! This function returns the max value among the cells of I that are part of the 
    //manhattan neighborhood of size K for cell at (i,j)
    /*!
      \param I the input matrix
      \param cell the coordinate of the cell for which the max value is to be calculated
      \param K the size of the manhattan neighborhood
      \return the max value for (i,j) among all cells (p,q) satisfying: |i-p|+|j-q| <= K
    */
using Matrix = std::vector<std::vector<int>>;
using Cell = std::pair<int,int>;

auto find_max_in_manhattan_neigh_k(const Matrix& I, const Cell& cell, 
const int K)
{
