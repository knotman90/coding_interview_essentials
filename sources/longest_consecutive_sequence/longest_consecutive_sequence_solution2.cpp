...
size_t find_longest_connected_component(std::vector<int>& L, Graph& g)
{
  ...
  for (const auto& l : L)
  {
    ...
    const auto length_left_from_l =
        find_longest_connected_component<Left>(g, l);
    // l is erased now: visit right from the next if exists and add 1 to account
    // for the first hop done here
    const auto length_right_from_l =
        g.contains(l + 1)
            ? find_longest_connected_component<Right>(g, l + 1) + 1
            : 0;
