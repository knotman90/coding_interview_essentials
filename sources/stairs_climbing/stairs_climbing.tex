...
The basic approach to solving this problem is always: \textit{Choose minimum/maximum path among all possible paths before the current state, then add the value for the current state.} but we will examine this in more detail in order to clarify what is meant by words such as \quotes{current} and \quotes{state} and also address a common variation on the problem. 
